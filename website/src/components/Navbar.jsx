...
import { Trophy, Github, FileText, Scale, Sun, Moon, Menu, X, BookOpen } from 'lucide-react'
...
export default function Navbar() {
  ...
  const navLinkClass = (path) =>
    `flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${isActive(path)
      ? isDark
        ? 'bg-bb-accent/10 text-bb-accent shadow-[0_0_12px_rgba(0,170,255,0.15)]'
  ...
  const extLinkClass = `flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${isDark
    ? 'text-gray-400 hover:text-white hover:bg-bb-dark-300/50'
    : 'text-gray-500 hover:text-gray-900 hover:bg-bb-light-200'
    }`

  return (
    <nav className={`sticky top-0 z-50 backdrop-blur-2xl border-b transition-all duration-300 ${isDark
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <Link to="/" className="flex items-center gap-3 group">
            <div className="relative">
              <Scale className={`w-8 h-8 transition-all duration-300 group-hover:scale-110 ${isDark ? 'text-bb-accent' : 'text-bb-accent-dark'
                }`} />
              <div className={`absolute inset-0 w-8 h-8 rounded-full blur-xl transition-all group-hover:blur-2xl ${isDark ? 'bg-bb-accent/20 group-hover:bg-bb-accent/40' : 'bg-bb-accent-dark/15 group-hover:bg-bb-accent-dark/25'
                }`} />
            </div>
          ...
          <div className="hidden md:flex items-center gap-1">
            ...
            <button
              onClick={toggleTheme}
              className={`relative flex items-center justify-center w-9 h-9 rounded-lg transition-all duration-300 ${isDark
          ...
          <div className="flex items-center gap-2 md:hidden">
            <button
              onClick={toggleTheme}
              className={`flex items-center justify-center w-9 h-9 rounded-lg transition-all ${isDark
                ? 'text-gray-400 hover:text-yellow-300 hover:bg-bb-dark-300/50'
                : 'text-gray-500 hover:text-amber-500 hover:bg-bb-light-200'
                }`}
            >
        ...
        {/* Mobile Menu */}
        {mobileOpen && (
          <div className={`md:hidden py-4 border-t animate-slide-down ${isDark ? 'border-bb-dark-50/20' : 'border-bb-light-300/50'
            }`}>
